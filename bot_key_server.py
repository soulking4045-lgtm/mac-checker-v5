...
import hashlib, hmac, json, os, secrets, sqlite3, sys, time
...
def add_permission(key, user_identifier, granted_by="admin", device_limit=1, expires_days=None):
...
def cli_add_user():
    ...
    ed = input(f"{Y}Expires days (never): {N}").strip()
    add_permission(key=key, user_identifier=uid, device_limit=int(dl), expires_days=int(ed) if ed else None)
